...
class USDTSellList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      dataList: []
  ...
  adsFetch = async () => {
    fetch(
      "http://52.65.228.235/api/v1/advertisements?ad_type=fiat_ad&trade_type=sell&coin=USDT",
      {
        method: "GET",
        headers: undefined,
        body: undefined
          ...
          <TouchableOpacity key={i} onPress={() => this.navToPreOrder(i)}>
            <Card isDark="true">
              ...
              <CardContent
                ...
                qty={"QTY " + item.margin_price + " " + item.coin}
              />
            </Card>
          </TouchableOpacity>
        ))}
...
export default withNavigation(USDTSellList);
